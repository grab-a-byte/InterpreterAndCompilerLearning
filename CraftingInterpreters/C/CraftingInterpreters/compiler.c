...
static void declareVariable() {
  ...
  for (int i = current->localCount - 1; i >= 0; i--) {
    ...
    if (identifiersEqual(name, &local->name)) {
      error("Already an identifier with this name in scope");
    }
  }
...
static uint8_t parseVariable(const char *error) {
  ...
  if (!check(TOKEN_RIGHT_PAREN)) {
    do {
      ...
      if (current->function->arity > 255) {
        errorAtCurrent("Unable to have more than 255 parameters");
      }
...
static void whileStatement() {
  ...
  expressionStatement();
  consume(TOKEN_RIGHT_BRACE, "expected ')' after while condition");
...
static void returnStatement() {
  if (current->type == TYPE_SCRIPT) {
    error("cant return from a script");
  }
  if (match(TOKEN_SEMICOLON)) {
    emitReturn();
  } else {
...
static int addUpvalue(Compiler *compiler, int8_t index, bool isLocal) {
